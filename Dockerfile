...
# installing sscep
RUN apt-get update && \
  apt-get install -y git make gcc libssl-dev automake autoconf && \
  ...
  make install && \
  rm -rf sscep && \
  apt-get remove -y gcc libssl-dev libc6-dev gcc-8 linux-libc-dev cpp cpp-8 libgcc-8-dev libc-dev-bin automake autoconf && \
  apt-get clean && \
  rm -rf /var/lib/apt/lists/*
