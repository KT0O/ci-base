...
# installing the rest
RUN apt-get install -y apt-transport-https ca-certificates curl gnupg2 software-properties-common socat certbot cron jq rpm nginx python3 python3-pip aspell aspell-en aspell-fr qemu-user && \
...
# installing sscep
RUN apt-get update && \
  apt-get install -y git make gcc libssl-dev libtool automake autoconf && \
  ...
  make install && \
  rm -rf sscep && \
  apt-get remove -y gcc libssl-dev libc6-dev gcc-8 linux-libc-dev cpp cpp-8 libgcc-8-dev libc-dev-bin libtool automake autoconf && \
  apt-get clean && \
  rm -rf /var/lib/apt/lists/*
